...
![G2 で note.com の記事を読んでいる様子](screenshot.png)
...
```
 iPhone ─── Even Hub WebView (app/) ─── BLE ──▶ Even G2
              │   - コンパニオン UI
              │   - G2 ページレンダラ
              │
...
- 「ログイン状態を保持する」にチェックを入れた場合、`{email, password}` は
  Even Hub SDK の bridge.localStorage に保存されますが、これは iPhone の
  Even Realities アプリ内サンドボックスのローカルストレージで、外部に
  送信されません。チェックを外してログインするか、ログアウトを押すと
  消えます。
